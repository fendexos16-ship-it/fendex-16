...
import { Activity, Server, Database, RefreshCw, Zap, ShieldAlert, Archive, CheckCircle, XCircle, Rocket, Lock, Key, Clock, Shield, AlertTriangle, PlayCircle, Settings, FileText, AlertOctagon, Eye, Globe } from 'lucide-react';
import { Modal } from '../../components/Modal';
import { Input } from '../../components/Input';

export const SystemHealth: React.FC = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'HEALTH' | 'DR' | 'CONFIG' | 'SECURITY'>('HEALTH');
    ...
    <Layout>
      ...
      <div className="mb-8 flex justify-between items-center">
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg">
           ...
           <button onClick={() => setActiveTab('SECURITY')} className={`px-4 py-2 text-sm font-medium rounded-md ${activeTab === 'SECURITY' ? 'bg-white text-gray-900 shadow' : 'text-gray-500'}`}>Security</button>
      ...
      {activeTab === 'SECURITY' && (
         <div className="space-y-6">
            <div className="bg-slate-900 text-white rounded-lg p-6 shadow-md border border-slate-700">
               ...
               <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <div className="bg-slate-800 p-4 rounded border border-slate-600">
                     <p className="text-xs text-slate-400 uppercase font-bold mb-2 flex items-center">
                        <Globe className="h-3 w-3 mr-1" /> WAF Status
                     </p>
                     <div className="text-2xl font-bold text-green-400 mb-1">Active</div>
                     <div className="text-xs text-slate-500">Rules: SQLi, XSS, Path Traversal</div>
                  </div>
                  
                  {/* Rate Limits */}
                  <div className="bg-slate-800 p-4 rounded border border-slate-600">
                     <p className="text-xs text-slate-400 uppercase font-bold mb-2 flex items-center">
                        <Zap className="h-3 w-3 mr-1" /> Rate Limiting
                     </p>
                     <div className="text-2xl font-bold text-yellow-400 mb-1">Enforced</div>
                     <div className="text-xs text-slate-500">Auth: 5/min | API: 60/min</div>
                  </div>

                  {/* Password Policy */}
                  <div className="bg-slate-800 p-4 rounded border border-slate-600">
                     <p className="text-xs text-slate-400 uppercase font-bold mb-2 flex items-center">
                        <Key className="h-3 w-3 mr-1" /> IAM Policy
                     </p>
                     <div className="text-2xl font-bold text-blue-400 mb-1">Strict</div>
                     <div className="text-xs text-slate-500">Min 12 Chars + Lockout (5 attempts)</div>
                  </div>

                  {/* Session */}
                  <div className="bg-slate-800 p-4 rounded border border-slate-600">
                     <p className="text-xs text-slate-400 uppercase font-bold mb-2 flex items-center">
                        <Clock className="h-3 w-3 mr-1" /> Session
                     </p>
                     <div className="text-2xl font-bold text-purple-400 mb-1">8 Hours</div>
                     <div className="text-xs text-slate-500">Absolute Timeout</div>
                  </div>
               </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
               ...
               <div className="grid grid-cols-2 gap-4">
                   <div className="flex items-center justify-between p-3 bg-gray-50 rounded border border-gray-200">
                      <div>
                         <p className="font-bold text-sm">3PL API Keys</p>
                         <p className="text-xs text-gray-500">Storage: Encrypted at Rest</p>
                      </div>
                      <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded font-bold">SECURE</span>
                   </div>
                   <div className="flex items-center justify-between p-3 bg-gray-50 rounded border border-gray-200">
                      <div>
                         <p className="font-bold text-sm">Database Credentials</p>
                         <p className="text-xs text-gray-500">Access: Private VPC Only</p>
                      </div>
                      <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded font-bold">INTERNAL</span>
                   </div>
               </div>
               <div className="mt-4 p-3 bg-blue-50 text-blue-800 text-sm rounded border border-blue-100">
                  <strong>Note:</strong> Secrets are never exposed to the frontend. All sensitive operations are proxied via secure backend handlers.
               </div>
            </div>
         </div>
      )}
