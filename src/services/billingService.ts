...
export const billingService = {
  ...
  getBillableShipments: async (
    ...
    // Calculate Fees Preview (Base Only)
    let totalFees = 0;
  ...
  // 2. CREATE DRAFT INVOICE (With SLA Logic)
  createDraftInvoice: async (
    ...
    // Get Active Rate Card for SLA Rules
    const rateCards = await rateCardService.getClientRates({ role: UserRole.FOUNDER } as any);
    // Find active card covering end of period
    const rateCard = rateCards.find(c => c.clientId === clientId && c.status === 'ACTIVE' && period.end >= c.effectiveDate && (!c.expiryDate || period.end <= c.expiryDate));
    ...
    const slaAdjustments: Invoice['slaAdjustments'] = [];
    if (rateCard && rateCard.slaRules && rateCard.slaRules.length > 0) {
       // 1. Calculate Metrics
       const metrics = await performanceService.getClientMetrics(user, clientId, period.start, period.end);
       
       for (const rule of rateCard.slaRules) {
          let metricValue = 0;
          if (rule.metric === SlaMetric.D0) metricValue = metrics.d0Percent;
          else if (rule.metric === SlaMetric.RTO) metricValue = metrics.rtoPercent;
          else if (rule.metric === SlaMetric.COD_TAT) metricValue = metrics.avgCodVerifyHrs;
          
          let conditionMet = false;
          if (rule.condition === 'GREATER_THAN') conditionMet = metricValue > rule.threshold;
          else conditionMet = metricValue < rule.threshold;

          if (conditionMet) {
             // Calculate Adjustment Amount
             let amount = 0;
             if (rule.adjustmentType === 'FLAT') {
                amount = rule.value * shipments.length; // Per shipment flat rate
             } else {
                amount = (subtotal * rule.value) / 100; // Percent of total freight
             }
             
             // Check polarity: Premium Adds to Invoice, Penalty Deducts? 
             // Actually, usually:
             // Premium = Bonus to Fendex? Or Bonus to Client (Discount)?
             // Context: "Pricing Modifiers".
             // Premium for high D0 -> usually Fendex charges MORE. (Value Add)
             // Penalty for low D0 -> Fendex charges LESS (Discount/Penalty on Service Provider).
             
             // Rule Type: PREMIUM (Charge More), PENALTY (Charge Less / Discount)
             if (rule.type === 'PENALTY') amount = -amount;

             slaAdjustments.push({
                ruleId: rule.id,
                description: `${rule.type}: ${rule.metric} is ${metricValue.toFixed(1)}% (${rule.condition} ${rule.threshold})`,
    ...
    // Apply Adjustments to Subtotal
    let slaTotal = slaAdjustments.reduce((sum, adj) => sum + adj.amount, 0);

    // Apply Safety Cap
    const capPercent = rateCard?.slaCapPercent || 20; // Default 20% cap
    const maxAdjustment = (subtotal * capPercent) / 100;
    
    if (Math.abs(slaTotal) > maxAdjustment) {
       const original = slaTotal;
       slaTotal = slaTotal > 0 ? maxAdjustment : -maxAdjustment;
       slaAdjustments.push({
          ruleId: 'CAP',
          description: `Safety Cap Applied (${capPercent}%)`,
          amount: slaTotal - original
       });
    }

    let finalSubtotal = subtotal + slaTotal;
    if (finalSubtotal < 0) {
       finalSubtotal = 0; // Safety floor
       slaAdjustments.push({ ruleId: 'FLOOR', description: 'Zero Floor Enforcement', amount: -finalSubtotal }); // visual correction
    }

    const tax = finalSubtotal * 0.18; // 18% GST Hardcoded
    ...
    const draft: Invoice = {
       ...
       slaAdjustments, // Store breakdown
       generatedBy: user.id,
    ...
    await complianceService.logEvent('BILLING_OP', user, `Created Draft Invoice for ${client.name}`, { count: shipments.length, amount: totalAmount, slaAdj: slaTotal });
