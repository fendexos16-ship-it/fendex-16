...
const getClientsDb = (): Client[] => {
  ...
  return [
    { 
      ...
      onboardingChecklist: {
         legalNameVerified: true, contactPersonVerified: true, billingCycleSet: true, taxDetailsVerified: true, rateCardBound: true, pickupSlaAgreed: true
      },
      technicalReadiness: {
         webhookSignatureVerified: true, testShipmentLifecyclePassed: true, billingPreviewGenerated: true, invoiceSampleApproved: true
      },
      documentsSigned: ['MSA_LEGACY', 'SLA_LEGACY']
    },
    { 
      id: 'C002', 
      clientCode: 'C0002',
      ...
      createdAt: new Date().toISOString(),
      onboardingChecklist: {
         legalNameVerified: true, contactPersonVerified: true, billingCycleSet: true, taxDetailsVerified: true, rateCardBound: true, apiCredentialsGenerated: true, webhookConfigured: true
      },
      technicalReadiness: {
         webhookSignatureVerified: true, testShipmentLifecyclePassed: true, billingPreviewGenerated: true, invoiceSampleApproved: true
      },
      documentsSigned: ['MSA_LEGACY']
    }
  ];
};
...
export const clientService = {
  ...
  getClientById: async (id: string): Promise<Client | undefined> => {
    ...
    const creds = getCredentialsDb().find(c => c.clientId === id);
    const perms = getPermissionsDb().find(p => p.clientId === id);
    
    return { ...client, credentials: creds, permissions: perms };
  },
  ...
  checkPermission: async (clientId: string, action: keyof ClientPermissions): Promise<boolean> => {
    ...
    // Status check: Only LIVE allows Ops unless Testing is allowed for specific actions
    // Allow 'TESTING' status to work if environment is TEST
    if (client.status !== ClientStatus.LIVE && client.status !== ClientStatus.TESTING) return false;
  ...
  saveClient: async (user: User, clientData: Client): Promise<Client> => {
    ...
    if (clientData.id) {
      ...
      finalClient = { 
        ...
        technicalReadiness: DEFAULT_READINESS,
        documentsSigned: [],
        createdBy: user.id,
        createdAt: new Date().toISOString()
      };
    ...
    await complianceService.logEvent(
      ...
      { 
        clientId: finalClient.id, 
        type: finalClient.type, 
        code: finalClient.clientCode,
        newValue: isNew ? undefined : JSON.stringify(finalClient),
        entityType: 'CLIENT',
        entityId: finalClient.id
      }
    );
