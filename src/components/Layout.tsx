...
import { FendexIcon } from './FendexIcon';
import { 
  LogOut, 
  LayoutDashboard, 
  MapPin, 
...
const NAV_ITEMS: NavItem[] = [
  // Operational
  { name: 'Dashboard', href: '/', icon: LayoutDashboard, roles: [UserRole.FOUNDER, UserRole.FINANCE_ADMIN, UserRole.AREA_MANAGER, UserRole.SALES_AGENT], section: 'Overview' },
  { name: 'Daily Ritual', href: '/founder/daily-brief', icon: Calendar, roles: [UserRole.FOUNDER], section: 'Overview' },
  ...
  { name: 'KPI Digest', href: '/founder/digest', icon: Mail, roles: [UserRole.FOUNDER], section: 'Overview' },
  
  // Sales
  { name: 'Sales Pitch Deck', href: '/sales/pitch-deck', icon: Presentation, roles: [UserRole.FOUNDER, UserRole.SALES_AGENT], section: 'Sales Enablement' },
  { name: 'Playbook & Scripts', href: '/sales/playbook', icon: Target, roles: [UserRole.FOUNDER, UserRole.SALES_AGENT], section: 'Sales Enablement' },
  ...
  { name: 'Billing & Payments', href: '/client/billing', icon: CreditCard, roles: [UserRole.CLIENT_VIEW], section: 'Portal' },
  ...
  // Masters
  { name: 'City Rollout', href: '/masters/cities', icon: Map, roles: [UserRole.FOUNDER], section: 'Masters' }, 
  ...
  { name: 'Rider Master', href: '/masters/rider', icon: Truck, roles: [UserRole.FOUNDER, UserRole.LMDC_MANAGER], section: 'Masters' },
  ...
  // Knowledge Base
  { name: 'SOPs & Runbooks', href: '/resources/knowledge-base', icon: BookOpen, roles: [UserRole.FOUNDER, UserRole.MMDC_MANAGER, UserRole.LMDC_MANAGER, UserRole.RIDER, UserRole.FINANCE_ADMIN, UserRole.AREA_MANAGER], section: 'Resources' },
  ...
  { name: 'Performance', href: '/admin/performance', icon: Zap, roles: [UserRole.FOUNDER], section: 'Admin' },
  { name: 'AI Watchtower', href: '/admin/anomalies', icon: Eye, roles: [UserRole.FOUNDER, UserRole.FINANCE_ADMIN], section: 'Admin' },
];
...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sections = ['Overview', 'Sales Enablement', 'Portal', 'Operations', 'MMDC Ops', 'LMDC Ops', 'Rider Tasks', 'Finance', 'Commercials', 'Masters', 'Resources', 'Admin'];
  ...
  return (
    <div className="min-h-screen bg-white flex font-sans text-gray-900">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-gray-200 fixed h-full z-20">
        <div className="flex items-center h-16 px-6 border-b border-gray-200">
           ...
           <div>
              ...
              <p className="text-[9px] font-bold text-gray-500 tracking-widest uppercase">Logistics</p>
           </div>
        </div>

        {/* Live Indicator - Light Mode Only */}
        <div className="bg-red-50 text-red-700 border-b border-red-100 text-[10px] font-bold text-center py-2 uppercase tracking-widest flex items-center justify-center">
           <Lock className="h-3 w-3 mr-1" /> PRODUCTION LIVE
        </div>
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50">
          <div className="flex items-center">
            ...
            <div className="ml-3 overflow-hidden">
              ...
              <p className="text-xs text-gray-500 truncate flex items-center"><Lock className="h-3 w-3 mr-1" /> {user?.role}</p>
            </div>
          </div>
          <button 
      ...
      {/* Main Content Area */}
      <main className="flex-1 md:ml-64 flex flex-col min-h-screen bg-gray-50/30">
         {/* Incident Banner */}
         {incidentMode && (
         ...
         <div className="md:hidden bg-white border-b border-gray-200 p-4 flex justify-between items-center sticky top-0 z-40">
            <div className="flex items-center gap-2">
               ...
               <span className="bg-red-50 text-red-700 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase border border-red-100">LIVE</span>
            </div>
      ...
      {user?.role === UserRole.RIDER && (
         <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 flex justify-around p-2 z-30 pb-safe shadow-lg">
            <Link to="/" className={`flex flex-col items-center p-2 ${location.pathname === '/' ? 'text-blue-600' : 'text-gray-500'}`}>
               <LayoutDashboard className="h-5 w-5" />
               <span className="text-[10px] mt-1">Home</span>
            </Link>
            <Link to="/rider/pickups" className={`flex flex-col items-center p-2 ${location.pathname.includes('pickup') ? 'text-blue-600' : 'text-gray-500'}`}>
               <Truck className="h-5 w-5" />
               <span className="text-[10px] mt-1">Pickup</span>
            </Link>
            <Link to="/rider/deliveries" className={`flex flex-col items-center p-2 ${location.pathname.includes('deliveries') ? 'text-blue-600' : 'text-gray-500'}`}>
               <Package className="h-5 w-5" />
               <span className="text-[10px] mt-1">Del</span>
            </Link>
            <Link to="/rider/rvp" className={`flex flex-col items-center p-2 ${location.pathname.includes('rvp') ? 'text-blue-600' : 'text-gray-500'}`}>
               <RotateCcw className="h-5 w-5" />
               <span className="text-[10px] mt-1">RVP</span>
            </Link>
            <Link to="/rider/cod" className={`flex flex-col items-center p-2 ${location.pathname.includes('cod') ? 'text-blue-600' : 'text-gray-500'}`}>
