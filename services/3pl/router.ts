...
export const threePlRouter = {
  ...
  handleInbound: async (
    ...
    try {
      ...
      // 8. SUCCESS LOG
      await complianceService.logEvent('API_SUCCESS', { id: client.id, role: 'CLIENT' }, action, {
      ...
      // 9. FAILURE LOG
      await complianceService.logEvent('API_BLOCKED', { id: client?.id || 'UNKNOWN', role: 'CLIENT' }, action, {
        timestamp: new Date().toISOString(),
  ...
  executeOutbound: async (
    ...
    try {
       ...
       await complianceService.logEvent('API_OUTBOUND_SUCCESS', { id: client.id, role: 'CLIENT' }, action, {
          timestamp: new Date().toISOString(),
          clientId: client.id,
          action,
       ...
       await complianceService.logEvent('API_OUTBOUND_FAILED', { id: client.id, role: 'CLIENT' }, action, {
          timestamp: new Date().toISOString(),
          clientId: client.id,
          action,
          provider: 'ROUTER',
